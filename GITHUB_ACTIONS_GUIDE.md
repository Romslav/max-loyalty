...
## 🔍 ЧАСТЬ 6: КАК МОНИТОРИТЬ СТАТУС ТЕСТОВ

### Где смотреть результаты тестов?

#### Способ 1️⃣: На странице PR

```
1. Перейди на GitHub
   https://github.com/Romslav/max-loyalty/pull/6

2. Скролли вниз до "Checks"
   Там будут все статусы:
   ✅ or ❌ Tests (Node 18.x)
   ✅ or ❌ Tests (Node 20.x)
   ✅ or ❌ Code Quality

3. Клик на статус → видишь детали
```

#### Способ 2️⃣: На странице Actions

```
1. Перейди сюда:
   https://github.com/Romslav/max-loyalty/actions

2. Видишь список всех workflows

3. Выбери нужный workflow

4. Видишь все шаги и результаты

5. Клик на шаг → видишь логи
```

#### Способ 3️⃣: Прямой линк на workflow

```
Когда ты делаешь push, GitHub создаёт прямую ссылку
На Actions страницу твоего конкретного запуска.

Обычно ссылка выглядит так:
https://github.com/Romslav/max-loyalty/actions/runs/123456789
```

### Что означают статусы?

```
🟡 PENDING/RUNNING    → Робот еще работает, жди
🟢 SUCCESS/PASSED     → ✅ Все OK, можешь мёржить
🔴 FAILED/ERROR       → ❌ Что-то сломалось
⚫ SKIPPED             → Шаг был пропущен
⚪ NEUTRAL             → Невозможно определить
```

### Обычное время выполнения

```
- npm ci (установка)     → 30-45 сек
- npm run test (тесты)   → 10-15 сек
- npm run type-check     → 5-10 сек
- npm run lint           → 5-10 сек
- npm run test:coverage  → 5 сек

Итого: 60-90 секунд максимум (обычно меньше)
```

---

## 💡 ЧАСТЬ 7: САМЫЕ ЧАСТЫЕ ВОПРОСЫ
...
### Q: Что делать если тесты зависают?
**A:** Обычно это не случается. Но если случилось:
1. Подожди 5 минут
2. Если не помогло, перезапусти workflow
3. Или попроси помощь у Romslav

### Q: Как я узнаю какие конкретно тесты упали?
**A:** На странице workflow логов:
1. Перейди на Actions
2. Выбери нужный workflow run
3. Кликни на "Tests" шаг
4. Видишь красным какие тесты упали
5. Там же есть exact error message

---

## 🎓 ЧАСТЬ 8: ВИЗУАЛЬНАЯ СХЕМА
...
# МОНИТОРИТЬ СТАТУС
Открой PR на GitHub:
https://github.com/Romslav/max-loyalty/pull/<PR_NUMBER>

Или Actions страницу:
https://github.com/Romslav/max-loyalty/actions
```
...
*Версия: 2.1 - с расширенным мониторингом*
*Дата: 20.01.2026*
*Обновлено: 20.01.2026 23:33 MSK*
